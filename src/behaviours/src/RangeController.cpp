...
bool RangeController::HasWork() {
    ...
    if (enabled && range != NULL && !range->isInside(current_location)) {
        cout << "Range controller has work:  current location (" << current_location.x << ", " << current_location.y
             << ")" << endl;
        has_work = true;
...
void RangeController::setBacktrackDistance(float backtrack_distance) {
    if (backtrack_distance <= 0)
        throw RangeControllerInvalidParameterException(" The backtrack distance must be positive.");
