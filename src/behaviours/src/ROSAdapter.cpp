    ...
    if (!initilized) {
        ...
        if (timerTimeElapsed > startDelayInSeconds) {
            ...
            loggerMessage = "currentLocation(x,y,theta) = (" + std::to_string(currentLocation.x)
                            ...
                            + ", " + std::to_string(currentLocationMap.y) + ", " +
                            std::to_string(currentLocationMap.theta) + ")";
        ...
        switch (shape_type) {
            case 1: // Circle
            {
                if (message.data.size() != 4)
                    throw ROSAdapterRangeShapeInvalidTypeException(
                            "Wrong number of parameters for circle shape type in ROSAdapter.cpp:virtualFenceHandler()");
            ...
            case 2: // Rectangle
            {
                if (message.data.size() != 5)
                    throw ROSAdapterRangeShapeInvalidTypeException(
                            "Wrong number of parameters for rectangle shape type in ROSAdapter.cpp:virtualFenceHandler()");
            ...
            default: { // Unknown shape type specified
                throw ROSAdapterRangeShapeInvalidTypeException(
                        "Unknown Shape type in ROSAdapter.cpp:virtualFenceHandler()");
            }
        }
    }
}
...
void transformMapCentertoOdom() {
    ...
    try { //attempt to get the transform of the center point in map frame to odom frame.
        tfListener->waitForTransform(publishedName + "/map", publishedName + "/odom", ros::Time::now(),
                                     ros::Duration(1.0));
