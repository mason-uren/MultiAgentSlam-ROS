...
Result SearchController::DoWork() {
  ...
  else if (attemptCount >= 5 || attemptCount == 0) 
  {
    ...
    if (first_waypoint)
    {
      ...
      searchLocation.x = currentLocation.x + (24 * cos(searchLocation.theta));
      searchLocation.y = currentLocation.y + (24 * sin(searchLocation.theta));
    }
    else
    {
      //select new heading from Gaussian distribution around current heading
      searchLocation.theta += (M_PI / 2); //45 degrees in radians
      searchLocation.x = currentLocation.x + (24 * cos(searchLocation.theta));
      searchLocation.y = currentLocation.y + (24 * sin(searchLocation.theta));
    }
