...
float PID::PIDOut(float calculatedError, float setPoint) {
    ...
    float sum = 0;
    for (int i = 0;
         i < integralErrorHistArray.size(); i++) //sum the array to get the error over time from t = 0 to present.
    {
    ...
    if (fabs(I) > config.integralMax ||
        fabs(P) > config.antiWindup) //reset the integral to 0 if it hits its cap of half max PWM
    {
        integralErrorHistArray.clear();
        integralErrorHistArray.resize(config.integralErrorHistoryLength, 0.0);
        I = 0;
