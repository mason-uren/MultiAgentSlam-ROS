...
byte wristPin = 12;
int fingerMin =
    800; // if you want to shift 0 to a new location raise min; this is closed
...
int cpr = 8400; //"cycles per revolution" -- number of encoder increments per
                //one wheel revolution

// Serial (USB <--> Intel NUC)
String rxBuffer;
unsigned long watchdogTimer =
    1000; // fail-safe in case of communication link failure (in ms)
...
LPS pressure;
Movement move = Movement(rightSpeedPin, rightDirectionA, rightDirectionB,
                         leftSpeedPin, leftDirectionA, leftDirectionB);
Odometry odom = Odometry(rightEncoderA, rightEncoderB, leftEncoderA,
                         leftEncoderB, wheelBase, wheelDiameter, cpr);
Servo fingers;
Servo wrist;
...
void setup() {
  Serial.begin(115200);
  while (!Serial) {
  } // wait for Serial to complete initialization before moving on
...
void parse() {
  if (rxBuffer == "v") {
    ...
    int c_microsec = centerUSValue * 57; // convert back to microseconds
    if (c_microsec > 18867) { // If it's larger than 18867(Max time it takes for
                              // ultrasonic to take), set it to 18867
      c_microsec = 18867;
    }
    int c_micro_remaining =
        18867 - c_microsec; // Remove the time it took from the max time

    delayMicroseconds(c_micro_remaining); // Delay by that ammount
    ...
    // int l_microsec = leftUSValue * 57;
    // int l_micro_remaining = 18867 - l_microsec;

    // delayMicroseconds(l_micro_remaining);
    ...
    // int r_microsec = rightUSValue * 57;
    // int r_micro_remaining = 18867 - r_microsec;

    // delayMicroseconds(r_micro_remaining);
...
String updateIMU() {
  ...
  if (!gyroscope.timeoutOccurred() &&
      !magnetometer_accelerometer.timeoutOccurred()) {
    ...
    LSM303::vector<float> linear_acceleration = {acc.y * 0.061 / 1000 * 9.81,
                                                 -acc.x * 0.061 / 1000 * 9.81,
                                                 acc.z * 0.061 / 1000 * 9.81};

    // Convert gyroscope digits to millidegrees per second, then to degrees per
    // second, and finally to radians per second
    L3G::vector<float> angular_velocity = {gyro.y * 8.75 / 1000 * (PI / 180),
                                           -gyro.x * 8.75 / 1000 * (PI / 180),
                                           gyro.z * 8.75 / 1000 * (PI / 180)};

    // Combine normalized magnetometer and accelerometer digits to produce Euler
    // angles, i.e. pitch, roll, and yaw
    LSM303::vector<float> orientation = {(float)mag.x, (float)mag.y,
                                         (float)mag.z};
    orientation.x -= (magnetometer_accelerometer.m_min.x +
                      magnetometer_accelerometer.m_max.x) /
                     2;
    orientation.y -= (magnetometer_accelerometer.m_min.y +
                      magnetometer_accelerometer.m_max.y) /
                     2;
    orientation.z -= (magnetometer_accelerometer.m_min.z +
                      magnetometer_accelerometer.m_max.z) /
                     2;
    LSM303::vector_normalize(&orientation);
    float roll =
        atan2(linear_acceleration.y, sqrt(pow(linear_acceleration.x, 2) +
                                          pow(linear_acceleration.z, 2)));
    float pitch =
        -atan2(linear_acceleration.x, sqrt(pow(linear_acceleration.y, 2) +
                                           pow(linear_acceleration.z, 2)));
    float yaw = atan2(-orientation.y * cos(roll) + orientation.z * sin(roll),
                      orientation.x * cos(pitch) +
                          orientation.y * sin(pitch) * sin(roll) +
                          orientation.z * sin(pitch) * cos(roll)) +
                PI;
    orientation = {roll, pitch, yaw};

    // Append data to buffer
    String txBuffer =
        String(linear_acceleration.x) + "," + String(linear_acceleration.y) +
        "," + String(linear_acceleration.z) + "," + String(angular_velocity.x) +
        "," + String(angular_velocity.y) + "," + String(angular_velocity.z) +
        "," + String(orientation.x) + "," + String(orientation.y) + "," +
...
String updateOdom() {
  ...
  txBuffer = String(odom.x) + "," + String(odom.y) + "," + String(odom.theta) +
             "," + String(odom.vx) + "," + String(odom.vy) + "," +
             String(odom.vtheta);
...
void imuInit() {
  ...
  magnetometer_accelerometer.m_min =
      (LSM303::vector<int16_t>){-113, -2726, -2197};
  magnetometer_accelerometer.m_max =
      (LSM303::vector<int16_t>){6767, 2859, 3224};
